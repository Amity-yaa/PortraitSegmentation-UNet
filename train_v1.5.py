...
if __name__ == '__main__':
    ...
    load_optim = True
    transform = Transforms(target_shape=(320, 180), convert2gray=0.3)
    ...
    model = UNet(3, out_channels=8, sigmoid_output=sigmoid_output, residual=True).cuda()
    optimizer = torch.optim.SGD(model.parameters(), 1e-3)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[2, 10, 18, 24, 30], gamma=0.1)
            ...
            for i, (x, y) in enumerate(train_dataloader):
                ...
                if iter_counter and iter_counter % eval_iters == 0:
                    ...
                    with torch.no_grad():
                        ...
                        TbWriter.add_image('val/image', x_val.detach().cpu().numpy()[0], iter_counter)
                        TbWriter.add_image('val/prediction', out_val.detach().cpu().numpy()[0], iter_counter)
                        del mious
                ...
                if iter_counter % log_iters == 0:
                    ...
                    for param_key in model_params:
                        TbWriter.add_histogram('net/{}'.format(param_key), model_params[param_key])
