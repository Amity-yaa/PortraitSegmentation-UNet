...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels=64, sigmoid_output=True, residual=False):
        super().__init__()
        self.down_blocks = []
        self.up_blocks = []
        self.upsamplings = []
        out_channels_first = out_channels
        self.down_block1 = UNetBlock(in_channels, out_channels, residual=residual)
        self.up_block1 = UNetBlock(out_channels * 2, out_channels, residual=residual)
        self.upsampling1 = UpsamplingBlock(out_channels * 2, out_channels)
        in_channels = out_channels
        out_channels *= 2

        self.down_block2 = UNetBlock(in_channels, out_channels, residual=residual)
        self.up_block2 = UNetBlock(out_channels * 2, out_channels, residual=residual)
        self.upsampling2 = UpsamplingBlock(out_channels * 2, out_channels)
        in_channels = out_channels
        out_channels *= 2

        self.down_block3 = UNetBlock(in_channels, out_channels, residual=residual)
        self.up_block3 = UNetBlock(out_channels * 2, out_channels, residual=residual)
        self.upsampling3 = UpsamplingBlock(out_channels * 2, out_channels)
        in_channels = out_channels
        out_channels *= 2

        self.down_block4 = UNetBlock(in_channels, out_channels, residual=residual)
        self.up_block4 = UNetBlock(out_channels * 2, out_channels, residual=residual)
        ...
        self.bottleneck = UNetBlock(in_channels, out_channels, residual=residual)
