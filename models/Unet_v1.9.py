...
v1.8 这个版本是加入了Norm的可选机制，但是实验完后发现，norm=None时的模型符合预期，但是norm=InstanceNorm时，residual_convbn的norm依然是BN，没有正确修改为InstanceNorm，因此将这一版本备份，并将在新版本中修正此bug

v1.9 当前版本修正了v1.8的bug.
"""
import torch
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels=64, activation=nn.ReLU, sigmoid_output=True, residual=False, norm=nn.BatchNorm2d):
        super().__init__()
        self.down_blocks = []
        self.up_blocks = []
        self.upsamplings = []
        ...
        self.down_block1 = UNetBlock(in_channels, out_channels, activation=activation, residual=residual, norm=norm)
        self.up_block1 = UNetBlock(out_channels * 2, out_channels, activation=activation, residual=residual, norm=norm)
        self.upsampling1 = UpsamplingBlock(out_channels * 2, out_channels)
        in_channels = out_channels
        out_channels *= 2

        self.down_block2 = UNetBlock(in_channels, out_channels, activation=activation, residual=residual, norm=norm)
        self.up_block2 = UNetBlock(out_channels * 2, out_channels, activation=activation, residual=residual, norm=norm)
        self.upsampling2 = UpsamplingBlock(out_channels * 2, out_channels)
        in_channels = out_channels
        out_channels *= 2

        self.down_block3 = UNetBlock(in_channels, out_channels, activation=activation, residual=residual, norm=norm)
        self.up_block3 = UNetBlock(out_channels * 2, out_channels, activation=activation, residual=residual, norm=norm)
        self.upsampling3 = UpsamplingBlock(out_channels * 2, out_channels)
        in_channels = out_channels
        out_channels *= 2

        self.down_block4 = UNetBlock(in_channels, out_channels, activation=activation, residual=residual, norm=norm)
        self.up_block4 = UNetBlock(out_channels * 2, out_channels, activation=activation, residual=residual, norm=norm)
        ...
        self.bottleneck = UNetBlock(in_channels, out_channels, activation=activation, residual=residual, norm=norm)
...
class UNetBlock(nn.Module):
    def __init__(self, in_channels, out_channels, activation=nn.ReLU, residual=False, norm=nn.BatchNorm2d):
        super(UNetBlock, self).__init__()
        conv1 = nn.Conv2d(in_channels, out_channels, 3, padding=1)
        conv2 = nn.Conv2d(out_channels, out_channels, 3, padding=1)
        # bn = nn.BatchNorm2d(out_channels)
        act = activation()
        norm = norm(out_channels) if norm is not None else norm
        seq = [conv1, norm, act, conv2, norm, act]
        seq = [i for i in seq if i is not None]

        self.block = nn.Sequential(*seq)  # block的决定代码

        if residual and (in_channels != out_channels):
            seq_residual = [nn.Conv2d(in_channels, out_channels, 1), norm]
            seq_residual = [i for i in seq_residual if i is not None]
            self.residual_path = nn.Sequential(*seq_residual)
    ...
    def forward(self, x):
        x1 = x
        x = self.block(x)
        if hasattr(self, 'residual_path'):
            x1 = self.residual_path(x1)
        if self.residual:
            x = x1 + x
        return x
