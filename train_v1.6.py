...
if __name__ == '__main__':
    ...
    load_optim = True
    transform = Transforms(target_shape=(640,360), convert2gray=0)
    ...
    optimizer = torch.optim.Adam(model.parameters(), 1e-4, weight_decay=0.005)
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[5, 10, 18, 24, 30], gamma=0.1)
            ...
            for i, (x, y) in enumerate(train_dataloader):
                ...
                if iter_counter and iter_counter % save_iters == 0:
                    ...
                    model_path = 'Model_{}_Epoch-{}_Iters-{}_loss-{:.4f}_criterionLoss-{:.4f}_IoULoss-{:.4f}_IoU-{:.4f}.pth'.format(
                        model_name, epoch, iter_counter, loss.item(), loss_val, miou, -np.log(miou))
